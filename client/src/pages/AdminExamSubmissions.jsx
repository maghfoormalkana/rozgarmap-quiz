...
const AdminExamSubmissions = () => {
  ...
  const exportToCSV = () => {
    ...
    // Added 'Roll No' to headers
    const headers = ['Date', 'Category', 'Student Name', 'Roll No', 'Email', 'Contact', 'Qualification', 'Institution', 'Score (%)'];
    const csvData = submissions.map(sub => [
      new Date(sub.createdAt).toLocaleDateString(),
      sub.categoryName,
      sub.fullName,
      sub.rollNo || 'N/A', // Added Roll No mapping
      sub.email,
      sub.contactNo,
      sub.qualification,
      sub.schoolCollege,
  ...
  const filteredSubmissions = submissions.filter(sub => 
    ...
    sub.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
    sub.categoryName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (sub.rollNo && sub.rollNo.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-slate-700 flex items-center gap-2">
          ...
          <input 
            type="text"
            placeholder="Search by student name, roll no, email, or category..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-transparent border-none focus:ring-0 text-gray-900 dark:text-white placeholder-gray-400"
          />
