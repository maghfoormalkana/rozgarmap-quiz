...
const QuizPage = () => {
  ...
  const { 
    ...
    selectAnswer, goToQuestion, nextQuestion, prevQuestion, submitQuiz
  } = useQuiz(questions, quizSetup.quizTime || 30)

  useEffect(() => { 
    if (!quizSetup.studentName) { navigate('/quiz-setup'); return } 
    fetchQuestions() 
  }, [categoryId])
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 flex gap-6">
        <div className="flex-1">
          <div className="card p-6 sm:p-8">
            <div className="mb-8">
              <span className="inline-block px-3 py-1 rounded-full bg-rozgar-blue/10 dark:bg-rozgar-blue/20 text-rozgar-blue text-xs font-semibold mb-4">
                ...
                return (
                  <button 
                    ...
                    className={`w-full text-left p-4 rounded-xl border-2 transition-all duration-200 flex items-center gap-3 ${
                      isSelected 
                        ? 'border-rozgar-blue bg-rozgar-blue/5 dark:bg-rozgar-blue/10 shadow-md' 
                        : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700/50'
                    }`}
                  >
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${
                      isSelected ? 'border-rozgar-blue bg-rozgar-blue' : 'border-gray-300 dark:border-gray-500'
                    }`}>
                      {isSelected && <div className="w-2.5 h-2.5 rounded-full bg-white" />}
                    </div>
                    <span className={`text-base ${isSelected ? 'font-medium text-rozgar-blue' : 'text-gray-700 dark:text-gray-300'}`}>
                      {option}
                    </span>
                  </button>
                )
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-rozgar-blue text-white font-medium hover:bg-rozgar-blue-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                ...
                return (
                  <button 
                    ...
                    className={`w-10 h-10 rounded-lg text-sm font-medium transition-all ${
                      isCurrent 
                        ? 'bg-rozgar-blue text-white shadow-md scale-110' 
                        : isAnswered 
      ...
      <Modal isOpen={showSubmitModal} onClose={() => setShowSubmitModal(false)} title="Submit Quiz?" size="sm">
        <div className="text-center">
          ...
          {answeredCount < totalQuestions && (
            <p className="text-sm text-orange-500 mb-4">{totalQuestions - answeredCount} question(s) remaining. Are you sure?</p>
          )}
          <div className="flex gap-3 mt-6">
            ...
            <button 
              ...
              className="flex-1 px-4 py-2.5 rounded-lg bg-rozgar-red text-white font-medium hover:bg-rozgar-red-dark transition-colors"
            >
              Submit Now
            </button>
          </div>
      ...
      <Modal isOpen={showTimeUpModal} onClose={() => {}} title="Time's Up!" size="sm">
        <div className="text-center">
          ...
          <button 
            onClick={handleSubmit} 
            className="w-full px-4 py-2.5 rounded-lg bg-rozgar-blue text-white font-medium hover:bg-rozgar-blue-dark transition-colors"
          >
            View Results
          </button>
        </div>
